...
function space_dynamic_scripts() {
    ...
    wp_enqueue_style('space-dynamic-style', get_template_directory_uri() . '/assets/css/templatemo-space-dynamic.css');
...
function space_dynamic_customize_register($wp_customize) {
    ...
    $wp_customize->add_section('hero_section', array(
        'title' => __('Hero Section', 'space-dynamic'),
        'priority' => 30,
    ));
    
    $wp_customize->add_setting('hero_title', array(
        'default' => 'We Make Digital Ideas & SEO Marketing',
...
// Custom Post Type for SEO Services
function create_seo_services_post_type() {
    register_post_type('services',
        array(
            'labels' => array(
                'name' => __('Seo Services'),
                'singular_name' => __('Seo Service')
            ),
            'public' => true,
            'has_archive' => true,
            'supports' => array('title', 'editor', 'thumbnail', 'excerpt'),
...
add_action('init', 'create_seo_services_post_type');
...
function service_icon_callback($post) {
    ...
    echo '<input type="text" name="service_icon" value="' . $service_icon . '" size="50" placeholder="Enter icon class (e.g., fa fa-chart-line)" />';
}

function save_service_meta($post_id) {
    if (!isset($_POST['service_nonce']) || !wp_verify_nonce($_POST['service_nonce'], basename(__FILE__))) return;
...
function add_banner_image_customizer_settings($wp_customize) {
    
    // Add Hero Section Panel
    $wp_customize->add_section('hero_section', array(
        'title'    => __('Hero Banner Section', 'textdomain'),
        'priority' => 30,
    ));
    
    // Banner Right Image Setting
    ...
    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'banner_right_image', array(
        'label'    => __('Banner Right Image', 'textdomain'),
        'section'  => 'hero_section',
        'settings' => 'banner_right_image',
        'description' => __('Upload an image for the right side of the banner. Recommended size: 600x500px', 'textdomain'),
    )));
    ...
    $wp_customize->add_control('banner_image_alt', array(
        'label'    => __('Banner Image Alt Text', 'textdomain'),
        'section'  => 'hero_section',
        'type'     => 'text',
        'description' => __('Alt text for the banner image (important for SEO and accessibility)', 'textdomain'),
    ));
    
    // Existing Hero Text Settings (if not already added)
    if (!$wp_customize->get_setting('hero_subtitle')) {
        $wp_customize->add_setting('hero_subtitle', array(
            'default'           => 'Welcome to Space Dynamic',
            'sanitize_callback' => 'sanitize_text_field',
            'transport'         => 'refresh',
        ...
        $wp_customize->add_control('hero_subtitle', array(
            'label'    => __('Hero Subtitle', 'textdomain'),
            'section'  => 'hero_section',
            'type'     => 'text',
        ));
    }
    
    if (!$wp_customize->get_setting('hero_title')) {
        $wp_customize->add_setting('hero_title', array(
        ...
        $wp_customize->add_control('hero_title', array(
            ...
            'section'  => 'hero_section',
            'type'     => 'textarea',
            'description' => __('HTML tags like <em> and <span> are allowed', 'textdomain'),
        ));
    }
    
    if (!$wp_customize->get_setting('hero_description')) {
        $wp_customize->add_setting('hero_description', array(
            'default'           => 'Space Dynamic is a professional looking HTML template using a Bootstrap 5.',
            'sanitize_callback' => 'sanitize_textarea_field',
            'transport'         => 'refresh',
    ...
    $wp_customize->add_control('show_banner_image', array(
        'label'    => __('Show Banner Image', 'textdomain'),
        'section'  => 'hero_section',
        'type'     => 'checkbox',
        'description' => __('Check to show the banner image, uncheck to hide it', 'textdomain'),
    ));
    ...
    $wp_customize->add_control('banner_image_animation', array(
        'label'    => __('Banner Image Animation', 'textdomain'),
        'section'  => 'hero_section',
        'type'     => 'select',
        'choices'  => array(
            'fadeInRight'  => __('Fade In Right', 'textdomain'),
            'fadeInLeft'   => __('Fade In Left', 'textdomain'),
            'fadeInUp'     => __('Fade In Up', 'textdomain'),
            'fadeInDown'   => __('Fade In Down', 'textdomain'),
            'fadeIn'       => __('Fade In', 'textdomain'),
            'bounceInRight' => __('Bounce In Right', 'textdomain'),
            'slideInRight' => __('Slide In Right', 'textdomain'),
        ),
        'description' => __('Choose animation effect for the banner image', 'textdomain'),
    ));
}
add_action('customize_register', 'add_banner_image_customizer_settings');
...
// Live Preview JavaScript for Project Section
function project_section_customizer_live_preview() {
    wp_enqueue_script(
        'project-section-customizer',
        get_template_directory_uri() . '/assets/js/project-customizer.js',
        array('jquery', 'customize-preview'),
        '1.0.0',
        true
    );
}
add_action('customize_preview_init', 'project_section_customizer_live_preview');
            ...
            <div class="row">
                ...
                <div class="col-lg-6 wow fadeInRight" data-wow-duration="1s" data-wow-delay="0.2s">
                    <div class="section-heading">
                        <<?php echo esc_html($heading_tag); ?>><?php echo wp_kses_post($heading_text); ?></<?php echo esc_html($heading_tag); ?>>
...
function get_project_section_heading() {
    ...
    return '<' . esc_html($heading_tag) . '>' . wp_kses_post($heading_text) . '</' . esc_html($heading_tag) . '>';
}
...
function get_project_section_paragraph() {
    ...
    return '<p>' . wp_kses_post($paragraph_text) . '</p>';
}
